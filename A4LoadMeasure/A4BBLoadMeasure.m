...
static CFAbsoluteTime firstLoadStart = 0;
static NSString *_loadTimeLogStr = @"";
...
static void printLoadInfoWappers(void) {
    ...
    NSMutableString *loadTimeTempLogStr = [NSMutableString stringWithFormat:@"Total load time: %f milliseconds", totalDuration * 1000];
    ...
    for (LMLoadInfo *info in infos) {
        ...
        NSString *loadTimeStr = [NSString stringWithFormat:@"\n%40s load time: %f milliseconds",[clsname cStringUsingEncoding:NSUTF8StringEncoding], info.duration * 1000];
        [loadTimeTempLogStr appendString:loadTimeStr];
    }
    _loadTimeLogStr = loadTimeTempLogStr;
    printf("\n");
}
...
retry:
    do {
        ...
        IMP hookImp = imp_implementationWithBlock(^ {
            info->_start = CFAbsoluteTimeGetCurrent();// 插入开始时间
            ((void (*)(Class, SEL))objc_msgSend)(cls, hookSel);
            info->_end = CFAbsoluteTimeGetCurrent(); // 插入结束时间
            if (!--LMAllLoadNumber) printLoadInfoWappers();
        });
...
@implementation A4BBLoadMeasure

static CFAbsoluteTime _didFnishStartTime = 0;

+ (NSString *)loadTimeLogStr {
    return _loadTimeLogStr;
}
+ (NSTimeInterval)firstLoadStartTime {
//    @([A4BBLoadMeasure firstLoadStartTime] + kCFAbsoluteTimeIntervalSince1970));
    return  (firstLoadStart + kCFAbsoluteTimeIntervalSince1970) * 1000;
}

+ (void)makeDidFinishStartTime {
    _didFnishStartTime = CFAbsoluteTimeGetCurrent();
}
+ (NSTimeInterval)didFinishStartTime {
    return  (_didFnishStartTime + kCFAbsoluteTimeIntervalSince1970) * 1000;;
}
